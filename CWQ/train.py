...
def train(args):
    ...
    ent2id, rel2id, train_loader, val_loader,test_loader,relation_embedd = load_data(args.input_dir, args.bert_name, args.batch_size, args.rev)
    for k in relation_embedd:
        if isinstance(relation_embedd[k], torch.Tensor):
            relation_embedd[k] = relation_embedd[k].to(device)
    logging.info("Create model.........")
    model = REAN(args, ent2id, rel2id)
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode='min', patience=3, verbose=True, factor=0.85)
    ...
    for epoch in range(args.num_epoch):
        ...
        for iteration, batch in enumerate(train_loader):
            ...
            loss = model(*batch_device(batch, device),relation_embedd)
            optimizer.zero_grad()
        ...
        if (epoch+1)%5 == 0:
            val_acc = validate(args, model, val_loader, device,relation_embedd)
            test_acc = validate(args, model, test_loader, device,relation_embedd)
