...
def validate(args, model, data, device, relation_embedd,verbose = False):
    model.eval()
    count = 0
    correct = 0
    ...
    with torch.no_grad():
        for batch in tqdm(data, total=len(data)):
            ...
            # batch = tuple(batch)
            outputs = model(*batch_device(batch,device),relation_embedd) # [bsz, Esize]
...
def main():
    ...
    ent2id, rel2id, train_loader, val_loader, test_loader,relation_embedd = load_data(args.input_dir, 'bert-base-cased',45)
    ...
    if args.mode == 'vis':
        validate(args, model, val_loader, device,  relation_embedd,True)

    elif args.mode == 'val':
        validate(args, model, val_loader, device, relation_embedd)
        validate(args, model, test_loader, device, relation_embedd)
