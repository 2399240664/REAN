...
def load_data(input_dir, bert_name, batch_size, add_rev=False):
    ...
    if os.path.exists(cache_fn):
        ...
        with open(cache_fn, 'rb') as fp:
            ent2id, rel2id, train_data, dev_data, test_data,relation_embedd = pickle.load(fp)
            # ent2id, rel2id, train_data, test_data, relation_embedd = pickle.load(fp)
        ...
        for line in open(os.path.join(input_dir, 'relations.txt')):
            ...
            line2 =line1[-1]
            line3 = line1[-2]
            # relation_level.append(line.split('.')[-2])
            # relation = line.strip()
            relation = line2 + " "+line3
            relation = re.sub('\.|_', ' ', relation)
            rel2id[line.strip()] = len(rel2id)
            relation_e = tokenizer(relation.strip(), max_length=18, padding='max_length', return_tensors="pt")
        ...
        if add_rev:
            for line in open(os.path.join(input_dir, 'relations.txt')):
                ...
                relation = relation + '_rev'
                relation_e = tokenizer(relation, max_length=18, padding='max_length', return_tensors="pt")
                relation_embedd.append(relation_e)
        print(len(rel2id))
        ...
        dev_data = DataLoader(os.path.join(input_dir, 'dev_simple.json'), bert_name, ent2id, rel2id, 62, add_rev=add_rev)
        test_data = DataLoader(os.path.join(input_dir, 'test_simple.json'), bert_name, ent2id, rel2id, 62, add_rev=add_rev)

        with open(cache_fn, 'wb') as fp:
            pickle.dump((ent2id, rel2id, train_data, dev_data, test_data, relation_embedd), fp)
            # pickle.dump((ent2id, rel2id, train_data, test_data, relation_embedd), fp)

    return ent2id, rel2id, train_data, dev_data, test_data, relation_embedd
    # return ent2id, rel2id, train_data, test_data, relation_embedd
