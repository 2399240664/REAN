...
def load_data(input_dir, bert_name, batch_size):
    ...
    if os.path.exists(cache_fn):
        ...
        print('Read data...')
        ent2id = {}
        for line in open(os.path.join(input_dir, '2entity.txt')):
            l = line.strip()
            ent2id[l] = len(ent2id)
        # print(len(ent2id))
        # print(max(ent2id.values()))
        rel2id = {}
        for line in open(os.path.join(input_dir, '2relation.txt')):
            l = line.strip()
            rel2id[l] = len(rel2id)
        ...
        #     relation_embedd={k:l}
        relation_embedd = {k: torch.cat([q[k] for q in relation_embedd], dim=0) for k in relation_embedd[0]}
        train_data = DataLoader(input_dir, os.path.join(input_dir, 'QA/22_train.txt'), bert_name, ent2id, rel2id, batch_size,relation_embedd, training=True)
        # val_data = DataLoader(input_dir, os.path.join(input_dir, 'QA/2q_val.txt'), bert_name, ent2id, rel2id, batch_size, relation_embedd)
        test_data = DataLoader(input_dir, os.path.join(input_dir, 'QA/22_test.txt'), bert_name, ent2id, rel2id, batch_size,relation_embedd)
